...
Neovim plugin to automatically stop unused language-server-provider (LSP)  
servers and restart them again when neovim window gains focus.  
This prevents excesive RAM usage.

The use cases include:
* multiple nvim instances each running their own unique LSP instance
* you edit multiple files in different apps
* you read documentation elsewhere, e.g. on the terminal, web, pdf etc.
* you went off to get a tea /coffee ☕

**Default behavior**:
* Tabpages and windows inside - are traversed & checked for LSPs
* When window loses focus - forcibly stops LSP servers for associated filetypes 
* The |'diff'| windows and |'readonly'| buffers are ignored - `lsp-timeout` takes no action
* On focused writable windows (cursor put on) `lsp-timeout` finds and previously used and available LSPs 
...
* If you hook up into |LspAttach| and |LspDetach| events, make sure to store & clean up buffer-local variables or keymaps only once per every cycle to avoid nvim memory overflow
...
* **null-ls** and **none-ls**:
<br/>as of `lsp-timeout` v1.2.0 should work properly; if didn't, replace by `efm-languageserver` or run `:e` upon focusing or use workaround,
...
<br/>run `lspconfig["typescript-tools"].setup({ filetypes = })` in your `init.lua`
<br/>make sure that `filetypes` are specified in the setup config
